...
pub trait BarcodeScannerApi<BlockHash, Hash> {
    ...
    fn is_valid_barcode(&self, encoded_hash: Hash, at: Option<BlockHash>) -> Result<bool>;
}
...
impl<C, Block, Hash> BarcodeScannerApi<<Block as BlockT>::Hash, Hash> for BarcodeScanner<C, Block>
where
    Block: BlockT,
    C: Send + Sync + 'static,
    ...
    Hash: Codec,
{
    fn is_valid_barcode(
        ...
        api.is_valid_barcode(&at, encoded_hash)
            .map_err(|e| RpcError {
